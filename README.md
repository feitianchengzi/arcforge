...
ArcForge 是给 coding agent 使用的本地优先、GitHub 优先 skill 生命周期治理工作台。

它帮助个人开发者和小团队把 skills 从创建、验证、审计、分组、应用、漂移检查一路推进到团队共享或公开发布准备。CLI 和 Desktop 都是 agent 可以调度的工具，不是让用户背命令的主入口。

![ArcForge 治理理念图](docs/assets/arcforge-overview.svg)

## 从当前仓库安装 ArcForge

安装 ArcForge 的推荐方式不是先手动运行 CLI，而是把当前仓库交给 coding agent。
...
ArcForge 自身安装完成后，安装 skill 会引导你选择是否安装飞天橙子推荐 Skill 项目，并先解释两种模式：
...
使用 arcforge 检查已安装副本和来源仓库之间的 drift
使用 arcforge 准备 GitHub 和 ClawHub/OpenClaw 发布 checklist
```
...
ArcForge 最开始是一个很具体的本地工作流需求：我希望在 skills 被复制到 agent、分享给团队或准备公开发布之前，先有一个轻量的治理层。随着 Skill First 工作流进入核心范围，ArcForge 也覆盖 skill 被创建和验证之后如何进入审计、正式化和应用流程。
...
如果你也遇到类似的 skill 管理问题，欢迎 star、提 issue 或发 PR。真实需求越多，越值得把 ArcForge 从个人工作流工具继续维护成更认真可用的项目。

## 产品定位

ArcForge 不是 skill marketplace、公开 registry、搜索引擎、评分系统、付费分发平台、包管理器，也不是 agent runtime。
...
一句话：registry 用来发现和分发，installer 用来复制到 agent，ArcForge 用来判断哪些 skill 值得信任、如何分组、如何应用、如何发布。

更详细的逐项对比见 [对比说明](docs/comparison.md)。

## 工作方式

ArcForge 的新使用理念是 agent-first：
...
| ClawHub/OpenClaw 等 registry | 公开发布目标或发布准备检查对象，不是 ArcForge 要替代的系统。 |
...
当你需要在 skills 被复制到 agent、团队共享或公开发布之前，先做一层本地治理时，适合使用 ArcForge。
...
如果你只是想浏览公开 skills，或一次性给某个 agent 安装一个公开 skill，ArcForge 不是最短路径。那类需求应该优先使用 registry 或 installer。
...
当 skills 位于 `.codex/skills`、`.claude/skills`、`.cursor/skills` 这类项目内 agent 目录时，项目根目录仍然是治理根目录。ArcForge 会把 agent skill 目录作为 `--source-dir` 传给 CLI，让本地项目状态、Git 状态、应用来源记录和漂移报告都归属于真实项目，而不是隐藏的 agent 目录。

ArcForge 会把日常本地项目设置保存到 `~/.arcforge/projects`，避免 GitHub source checkout 因 profile 或 target 变化而变脏。项目根目录下遗留的 `arcforge.config.json` 会在需要时迁移到用户级项目状态。
...
![ArcForge 桌面端演示](docs/assets/arcforge-desktop-demo.gif)
...
使用视频：[ArcForge CLI 演示](docs/assets/arcforge-cli-demo.mp4)
