...
Agent Skill 接口是 SkillOps 面向 coding agent 的默认交互入口。用户位于当前项目目录时，可以通过 SkillOps skill 调用 CLI 和按需进入桌面端，完成项目内 skill 的审计、正式化、跨项目应用、漂移检查和远程共享。

SkillOps skill 在当前项目根目录的 `skills/skillops/` 下维护。该 skill 是产品自举资产，用于沉淀 SkillOps 自身的 agent 使用方式，并作为后续 CLI 与桌面端优化的场景基准。
...
SkillOps skill 使用一个主 skill，而不是多个分散 skill。主文件 `skills/skillops/SKILL.md` 保存通用规则和 workflow 选择，详细流程放在同级 `references/` 文件中。
...
用户能够在当前项目目录中通过 SkillOps skill 完成项目内 skill 的审计、归并、应用、漂移检查和远程共享准备。
