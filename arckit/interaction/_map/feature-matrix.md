...
| destinations/ | ✅ 已完成 | 标准 Agent 目标先检查漂移并审查三类 skill 的 availability-aware 计划；自定义目录保持 direct 路径，诊断、cleanup 和关系保存均在写入前确认 | 目标组合与漂移、目标 Sheet、三类模式计划、阻断诊断、cleanup 确认、应用结果与失败恢复 | arckit/spec/profile/skill-availability.md、arckit/spec/sources/skill-project-merge.md、src/ui/views/destinations.tsx、src/ui/main.tsx |
