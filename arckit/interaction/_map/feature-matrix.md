...
| destinations/ | ✅ 已完成 | 标准 Agent 目标先检查漂移并审查 availability-aware 计划；同名按需 skill 形成单一 catalog 候选，以 SemVer 决定升级并对降级或不可判定冲突 fail closed；项目适用性由 Agent assessment 或用户显式 override 提供，自定义目录保持 direct 路径 | 目标组合与漂移、目标 Sheet、单一 catalog 候选与版本决策、来源冲突选择、assessment/override、阻断诊断、旧目录 cleanup 确认、应用结果与失败恢复 | arckit/spec/profile/skill-availability.md、arckit/spec/sources/skill-project-merge.md、src/ui/views/destinations.tsx、src/ui/main.tsx |
