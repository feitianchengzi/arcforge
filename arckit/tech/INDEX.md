...
- workspace/ 工作区技术：本地项目状态、项目配置迁移、技能发现、共享资产发现、本地 Git 来源识别与工作区快照生成。✅
  - solution.md 工作区扫描方案：配置加载、frontmatter 解析、目录遍历、本地 Git 来源识别、来源更新检查和快照聚合。✅ (99行)
...
- sharing-ipc/ 共享与 IPC 技术：Electron preload 桥、应用数据目录注入、技能文件编辑、来源维护、权限预检、共享漂移、共享工作树和 Git/PR 执行。✅
  - solution.md 共享与 IPC 方案：安全桥接、来源下载与维护、发布计划、共享漂移、共享推送、CLI 修复、技能文件编辑和差异窗口。✅ (171行)
- cli/ 命令行技术：JSON 输出、命令参数、退出码、share/doctor 命令和桌面 --cli 模式。✅
  - solution.md CLI 方案：init、scan、audit、publish-plan、source、drift、apply-profile、share、doctor 命令和 GitHub Release 安装边界。✅ (109行)
- _shared/models/ 数据模型：跨领域 TypeScript DTO 与持久化配置结构。✅
  - SkillOpsConfig.yaml 工作区配置模型：来源目录、团队仓库、共享模式、配置组和目标组合列表。✅ (143行)
  - LocalProjectState.yaml 本地项目状态模型：项目身份、用户级配置覆盖和更新时间。✅ (66行)
...
- _shared/contracts/ IPC 契约：渲染层通过 preload 调用主进程和 core 能力。✅
  ...
  - source-update.yaml 来源更新契约：输入工作区根目录和确认状态，输出来源 ahead/behind 状态或快进更新结果。✅ (62行)
