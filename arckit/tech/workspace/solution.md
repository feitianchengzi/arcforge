...
工作区扫描方案以本地目录为输入，生成可供桌面端、CLI、归并和应用流程复用的工作区快照。方案聚合配置加载、技能发现、共享资产发现、审计执行和本地 Git 来源识别。

## 配置与本地项目状态

配置加载采用两层结构：默认配置和用户级本地项目状态。用户级本地项目状态位于 `~/.arcforge/projects/<project-key>.json`，由 CLI 和 Desktop 共同读写。
...
项目根目录下的 `arcforge.config.json` 是迁移输入，不是持久配置。系统发现该文件时，如果用户级项目状态不存在，先解析文件并保存为用户级本地项目状态；如果用户级项目状态已经存在，则不覆盖用户级状态。完成判断后系统删除项目根目录下的该文件。
...
本地 Git 来源识别返回工作区所属 Git 根目录、工作区相对 Git 根目录路径、当前分支和远端列表。

Git 来源识别结果嵌入 `WorkspaceSnapshot`，供独立 Git 更新检查、共享执行和桌面展示使用。

fast-forward-only 更新检查由 `core/source-update` 执行。工作区扫描方案只负责提供 Git 上下文，不暴露初始化命令。
...
该方案由 `workspace-scan` 契约暴露给桌面端。

CLI 的 `scan`、`audit`、`publish-plan`、`source`、`merge`、`applied`、`apply` 和 `drift` 命令均复用该方案。
