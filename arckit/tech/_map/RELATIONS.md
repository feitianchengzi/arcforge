...
- workspace/solution.md 生成 WorkspaceSnapshot，并提供来源更新检查与快进更新能力，供 audit/solution.md、profiles-sync/solution.md、sharing-ipc/solution.md 和 cli/solution.md 使用。
...
- cli/solution.md 通过命令编排层复用 workspace/solution.md、audit/solution.md、profiles-sync/solution.md、来源维护和共享 core 能力，并作为桌面端同等能力的执行入口。
...
- LocalProjectState.yaml 持有用户级项目配置覆盖，引用 SkillOpsConfig.yaml，并由 core/project-store 读写。
...
- SourceUpdateStatus.yaml 由 source-update 状态检查返回，SourceUpdateResult.yaml 由确认后的来源更新返回。
- EnvironmentStatus.yaml 由 system-environment 返回。
- SkillOpsConfig.yaml 包含用户级项目状态中的应用目标组合和共享目标组合。

## 契约关系

- workspace-scan 是桌面端大部分页面的数据入口。
- workspace-init 写入用户级 LocalProjectState 中的默认 SkillOpsConfig，并通常后接 workspace-scan。
...
- source-download 返回本地项目根路径，随后由 workspace-scan 打开；source-update 返回 GitHub 来源 checkout 的 ahead/behind 状态，并在确认后执行 fast-forward-only 更新。
