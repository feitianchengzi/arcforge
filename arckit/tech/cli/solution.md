...
参数使用名称查找方式读取。主要参数包括 `--root`、`--from`、`--to`、`--profile`、`--skills`、`--target`、`--target-path`、`--visibility`、`--repo`、`--message`、`--target-mode`、`--project-name`、`--same-repository`、`--same-repository-remote`、`--cache-dir`、`--mode`、`--agent`、`--agent-command`、`--timeout-ms`、`--save` 和 `--confirm`。
...
Skill 项目解析、归并和应用关系位于 `core/sources`。配置组应用和漂移位于 `core/profiles`。Agent 辅助审计位于 `core/agent-audit`。GitHub 来源解析、共享工作树、README 写入、配置合并和 Git 推送位于共享 core 模块。
