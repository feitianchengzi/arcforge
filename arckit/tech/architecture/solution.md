...
归并把当前项目技能复制到另一个 Skill 项目，并把当前项目应用关系写入用户级项目状态。来源 Skill 项目不需要全局登记。

共享执行会先扫描当前工作区，再同步共享工作树、生成共享 README 区块、提交并推送远端分支。用户本地共享偏好保存在用户级项目状态中，不写入来源 checkout。
...
该方案覆盖 `workspace-scan`、`workspace-add-remote`、`apply-run`、`apply-drift`、`share-plan`、`share-run`、`share-drift`、`system-environment` 和 `skill-file`。
