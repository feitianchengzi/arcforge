...
- 人工桥接执行包已写明预期实际读取的目标 skill 工作副本路径，并说明如何让新 agent 绑定该工作副本而不是同名用户级旧版。
- 第一段真实任务 prompt 只包含真实用户会说的任务、真实输入材料、工作区和写入边界；没有包含验证背景、诊断、观察重点、失败模式、验收标准、执行形态提示、总结格式、目标 skill 绝对路径或“你是验证执行者”这类测试控制语。
- 第一段真实任务 prompt 没有把主会话观察重点、刚修过的规则或预期失败模式伪装成“用户要求注意/避免/必须做到”的任务约束。
- 第一段真实任务 prompt 没有把目标 skill 应自行决定的落地方式伪装成用户要求，例如最小文件结构、本地结构检查、必须生成哪些文件或必须如何拆 reference。
- 收到事后总结后，已核对隔离 agent 实际读取的目标 skill 路径；如果读到同名旧版或其他路径，状态是验证绑定失败而不是目标 skill 验证通过。
