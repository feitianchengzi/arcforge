...
- 人工桥接执行包必须包含目标 skill 路径、工作区、允许写入边界、临时路径、用户操作步骤、当前状态 `awaiting_validation_transcript`、第一段真实任务 prompt 和第二段事后总结 prompt。
- 第一段真实任务 prompt 只能模拟用户真实会说的话；如果预期显式触发 skill，可写“执行 `<skill-name>` 做 `<task>`”，如果预期自动触发，就只写任务本身。
