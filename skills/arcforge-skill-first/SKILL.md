...
创建和维护目标 skill 时，保持“边界前移、正文正向执行”的结构。`description` 负责触发条件、适用边界、相邻 skill 的分工和不触发场景；正文从已经触发的场景开始，说明 agent 应如何理解输入、收集上下文、执行步骤、验证结果和交付输出。不要把一次协作中的用户纠错、失败复盘、业务临时约束或“不要像上次那样”的说明直接写进正文；先抽象成适用于同类任务的正向规则。执行型 skill 的正文不要用大量篇幅解释它与其他 skill 的区别，也不要用“先用 A / 不用 B / 与 C 的区别”来补救触发层不清晰的问题。
...
18. 目标 skill 的 `description` 承担触发条件、适用边界、相邻 skill 分工和不触发场景；执行正文只写被触发后的正向工作流。
19. 维护目标 skill 时，把用户纠错、失败复盘和一次性业务上下文抽象为可复用的正向规则；不要把纠错口吻、临时业务细节或“不要再……”类说明直接写进正文。
20. 执行型 skill 正文不要解释它和其他 skill 的差异；如果必须说明差异，先收敛到 frontmatter `description`、上层路由规则或 Skill First 的入口分工。
21. 保留必要的安全、权限、产品方向和破坏性操作边界；这些硬边界可以用明确禁止语句表达，但不要用普通纠错语句替代执行方法。
22. 目标 skill 默认使用用户当前请求语言；用户用中文提出需求时，`SKILL.md`、references 和 `agents/openai.yaml` 文案默认写中文。
23. 只有当用户明确要求英文、双语或指定语言时，才使用其他语言。
24. 当主 `SKILL.md` 变得拥挤时，把可复用细节放到同级一层 `references/` 文件中。
25. 新增 reference 时，必须在目标 `SKILL.md` 中写清楚何时读取它。
26. 如果需要 UI，必须说明 UI 承载什么决策、输入或预览，以及 UI 结果如何以结构化数据回到 agent 对话上下文、本地状态或下一条 CLI/server 动作。
27. Skill First 执行默认必须启动子代理做前测和复测；不需要用户额外确认，但如果用户明确要求不要启动或要求停止，或当前平台策略要求用户显式授权子代理，必须服从平台边界并记录原因。
28. 传给子代理的是目标 skill 路径、真实任务、工作区、临时路径和写入边界；不传主 agent 的诊断、预期答案、问题分类、修复思路或“请评价 skill 是否需要优化”的任务。
29. 子代理只负责像普通使用者一样执行任务并记录过程；不要要求子代理承担观察者、评审者、架构师或优化决策者角色。
30. 除非测试任务本身要求修改源码，否则要求子代理不要修改源文件。
31. 模拟项目、fixtures、输出文件使用临时路径。
32. 启动子代理前先确认当前环境是否有可用子代理工具；有可用工具且平台策略允许时必须真实启动子代理。
33. 只有当前环境没有可用子代理工具，或工具调用失败且无法恢复，或当前平台策略明确禁止未授权子代理时，才保存原始子代理 prompt，明确标注为“降级模拟”，并继续做主 agent 观察、问题分类、治理和复测。
34. `agents/openai.yaml` 不只写默认 prompt，还要同步目标 skill 的关键输出格式、边界规则、语言要求、能力单元建模要求、目标 skill 适配阈值、正文正向执行要求和已有实现承载发现要求。
35. 主 agent 把子代理执行记录或降级模拟记录转化成具体的 skill、reference、工具、CLI、server、UI、状态模型、已有实现接入或流程修复。
36. 有实质修复后至少再模拟一次，除非用户停止或任务明显很小。
37. 用户确认目标 skill 可用后，自动说明建议进入的 ArcForge 治理阶段，例如 audit、merge plan、profile、apply、drift 或 publish/share plan。
38. ArcForge 交接只做路由和下一步建议；不要在本 skill 中重新实现 ArcForge 的 audit、apply、drift、publish 或 share 逻辑。
39. 不要自动执行真实项目写入、目标目录替换、Git 更新、push、PR、远程分享或 registry 相关命令；这些必须交给 `arcforge` workflow 并按其确认规则执行。
40. 可以建议优先从低风险的 ArcForge 阶段开始，例如 scan、audit、merge plan、publish plan 或 share plan；涉及 run、apply、update、push、share 的动作必须先说明范围和风险并等待用户确认。
41. 如果用户只要求完成 Skill First 验证，不要强行推进 ArcForge 后续流程；最终汇报中给出可选的下一步即可。
...
13. 检查目标 skill 是否满足：`description` 管触发和边界，正文管正向执行；用户纠错已被抽象为可复用规则；执行正文没有用相邻 skill 差异说明补救路由不清。
14. 运行本地结构检查。
15. 主 agent 定义观察点和写入边界，然后启动子代理，让它使用目标 skill 完成真实任务并输出执行记录；只有没有可用子代理能力、工具无法恢复、平台策略不允许，或用户明确要求停止时，才按降级模拟记录执行。
16. 收集子代理执行记录；不要把子代理结论当成最终诊断。
17. 主 agent 将执行记录分类为：元 skill 越权执行、目标 skill 适配阈值、泛化 skill 抢占目标 skill、路由和入口分工、边界后置、纠错泄漏、业务耦合、正文负向补救、子代理 prompt 过载、能力包实现建模、已有实现承载发现、skill 表述、正式 skill 来源和复制流程、缺少 reference、缺少工具或 CLI/server/UI 能力、缺少状态或回传机制、不安全流程、测试环境问题或产品决策。
18. 做范围明确的修复。
19. 使用新的临时路径再次执行验证，或在用户要求停止时报告未复测。
20. 当执行路径跑通，或只剩已接受的产品缺口时，向用户确认目标 skill 是否可用。
21. 如果用户确认可用，说明建议进入的 ArcForge 治理阶段和原因；只引导到 `arcforge`，不绕过它的确认规则。
