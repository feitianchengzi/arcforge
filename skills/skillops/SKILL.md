...
使用这个 skill 时，把 SkillOps 理解为本地优先、GitHub 优先的 agent skill 生命周期治理工作台。`skillops-skill-first` 负责 skill 的创建、迭代和子代理验证；这个 `skillops` skill 负责接住验证后的治理工作，包括审计、正式化、profile、应用、漂移和发布准备。

SkillOps 不是 marketplace、公共 registry、搜索引擎、评分系统、付费分发平台或 agent runtime。GitHub 是 review、版本、release 和权限控制来源；ClawHub/OpenClaw、`skillshare`、`npx skills` 等是分发或安装相邻系统。
...
- Skill First 交接：接收 `skillops-skill-first` 验证通过的 skill，判断下一步应进入审计、正式化、profile、应用、漂移或发布准备。
...
10. 临时验证或子代理模拟必须使用临时项目路径，并设置 `SKILLOPS_HOME=/tmp/...` 或 `/private/tmp/...`，避免写入真实 `~/.skillops`。
11. 如果 CLI、Desktop 或 agent workflow 需要的能力本地未实现，直接说明缺口，并继续使用可用 fallback。

## 渐进加载

- 用户要理解 SkillOps 产品概念、阶段化治理或端到端路径时，读取 [references/capability-framework.md](references/capability-framework.md)。
