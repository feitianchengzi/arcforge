...
2. 说明将写入的真实用户级目标：
   ...
   - CLI shim: 默认 `~/.local/bin/arcforge`，或脚本按平台选择的用户级 bin 目录
...
- `--desktop install`：安装依赖、执行 `npm run build`，并写入用户级 `arcforge-desktop` launcher；这是默认模式。
...
- 不要删除仓库外的文件。脚本只替换目标 `arcforge`、`arcforge-skill-first` skill 目录和 `arcforge` shim。
