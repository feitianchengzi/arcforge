...
- 远程安装时为了读取仓库、计算 drift 或执行 apply 生成的 `/tmp`、`/private/tmp` checkout 是临时来源 checkout，不是本地维护源。只有用户明确选择导入、正式化或使用某个本地 Skill 项目持续维护时，才把该路径称为维护源。
...
- 目录位于当前项目 `sourceDir`、包含 `SKILL.md` 或已经完成归并，都不能证明它是临时副本。`cleanup-local` 只接受用户明确选中的目录，并限制在当前项目 `sourceDir` 内；执行前由 Agent说明 provenance 证据并确认具体目录。
...
- 关系记录归属 root：如果保存关系，必须单独列出，不得用当前 cwd 静默代替。
...
|---|---|---|---|---|---|---|
| 从远程 GitHub/Git Skill 项目安装到 Codex/Claude/Cursor 用户级目录 | 远程 URL 或 Git shorthand；可另列临时来源 checkout | 本轮无持久维护源，除非用户要先纳入当前项目 `targetDir` 或正式 Skill 项目治理 | 用户级 agent skill 目录 | 暂无 | `drift`，确认后 `apply --save`；需要先纳入维护时先 `import plan/run` | 需要选择远程源、skills、agent 类型或复核 diff 时打开 Skills/Import 或 Destinations |
...
- 删除本地 skill 目录必须是独立的显式选择；`workflow local-skill` 不自动追加清理阶段。`merge cleanup-local` 真实删除必须带 `--confirm`，并且只允许删除当前项目 `sourceDir` 下含 `SKILL.md` 的选定目录。
...
```text
来源/上游源: <已知/待确认/本轮无>
临时来源 checkout: <路径/本轮无；如存在，说明不是维护源>
维护源: <已知/待确认/本轮无持久维护源>
应用目标: <已知/待确认/本轮无>
共享目标: <已知/待确认/本轮无>
profile/skills: <已知/待确认>
关系记录: <保存/不保存/待确认/本轮无>
关系记录归属 root: <路径/本轮无；保存关系时必填，说明不是应用目标，也不一定是当前 cwd>
