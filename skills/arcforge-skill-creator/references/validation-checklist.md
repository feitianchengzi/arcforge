...
最终汇报前使用本清单。目标是确认目标 skill 被建模为可复用、可维护、可治理的能力单元，而不是只生成了一个标准文件夹或被预设为软件项目。本清单支持 `arcforge-skill-creator` 独立完成创建/维护；结束时必须判断本地停止、交给 `arcforge-skill-first` 验证、交给 `arcforge` 治理同步，或先验证再治理。
...
- 已由 Agent 判断能力性质；没有通过代码关键词或固定领域枚举替代语义判断。
- 已判断目标 skill 的最小承载：`SKILL.md`、reference、工具、CLI、API/MCP、server、UI、状态、schema、测试 fixture、回传机制。
...
- 已记录安装/可见副本、正式维护源证据和工作副本路径。
- 没有仅凭用户级、项目级、plugin cache 或当前会话加载路径宣称正式维护所有权。
- 如果目标来自正式 skill，未直接修改原始路径。
- 如果当前仓库本身就是正式 Skill 项目并直接修改维护源，已说明该路径为什么可视为正式维护源工作区。
- 工作副本位于用户授权的工作区或目标 Skill 项目 sourceDir；没有把当前项目根 `skills/` 当作所有场景的唯一位置。
