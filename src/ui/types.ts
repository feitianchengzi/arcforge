import type { AgentAuditProxyConfig, AppState, AppliedSourceRecord, ApplyFromSourceResult, ApplyProfileResult, AuditMode, CliInstallStatus, DriftReport, EnvironmentStatus, ImportSkillsPlan, ImportSkillsResult, InstalledSkillOrganizePlan, InstalledSkillOrganizeResult, InstalledSkillsInventory, InstalledSkillsScanOptions, MergePlan, MergeResult, PublishPlan, RecentWorkspace, ShareDeliveryMethod, SharePlanResult, ShareResult, ShareTargetMode, SkillAvailabilityOverride, SkillAvailabilityPlan, SkillEditorWindowContext, SkillFileDocument, SkillFileEntry, SkillProjectApplicabilityAssessment, ArcForgeConfig, SourceUpdateResult, SourceUpdateStatus, WorkspaceSnapshot } from "../shared/types";
...
declare global {
  interface Window {
    arcforge: {
      ...
      createSkillAvailabilityPlan: (options: { root: string; from?: string; profile?: string; skills?: string[]; agentTargetIds: string[]; projectTargetDirs?: string[]; availabilityOverrides?: SkillAvailabilityOverride[]; projectAssessments?: SkillProjectApplicabilityAssessment[] }) => Promise<SkillAvailabilityPlan>;
      applySkillAvailabilityPlan: (options: { root: string; from?: string; profile?: string; skills?: string[]; agentTargetIds: string[]; projectTargetDirs?: string[]; availabilityOverrides?: SkillAvailabilityOverride[]; projectAssessments?: SkillProjectApplicabilityAssessment[]; cleanupPaths?: string[]; save?: boolean; confirm: boolean; allowUnrelatedRoot?: boolean }) => Promise<ApplyFromSourceResult>;
      driftSkillAvailability: (options: { root: string; from?: string; profile?: string; skills?: string[]; agentTargetIds: string[]; projectTargetDirs?: string[]; availabilityOverrides?: SkillAvailabilityOverride[]; projectAssessments?: SkillProjectApplicabilityAssessment[] }) => Promise<DriftReport>;
