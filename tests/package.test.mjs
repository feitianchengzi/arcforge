...
test("cli-first share command is exposed to desktop and terminal entrypoints", async () => {
  ...
  assert.match(commands, /arcforge project resolve --name team-skills/);
  assert.match(commands, /arcforge project availability plan --root/);
  assert.match(commands, /project availability run/);
  assert.match(commands, /createSkillProjectAvailabilityPlan/);
  assert.match(commands, /executeSkillProjectAvailabilityPlan/);
  assert.match(commands, /Source availability update requires --confirm/);
  assert.match(commands, /--plan-digest/);
  assert.match(commands, /planChanged/);
  assert.match(commands, /arcforge workflow local-skill plan --root \. --skill review --to \.\.\/team-skills --install codex:user --share origin/);
...
test("cli plans real maintenance source workflows before local skill cleanup", async () => {
  ...
  assert.match(sources, /only deletes explicitly selected skill directories under the current project sourceDir/);
  assert.match(sources, /Supply an explicit Skill project path/);
  assert.doesNotMatch(sources, /Recommended maintenance source/);
  ...
  assert.match(cliOrchestration, /arcforge project availability plan/);
  assert.match(cliOrchestration, /arcforge project availability run/);
  ...
  assert.match(sourceTargetModel, /不能证明它是临时副本/);
  assert.match(sourceTargetModel, /不自动追加清理阶段/);
...
test("arcforge skill models source maintenance apply and share endpoints", async () => {
  ...
  const sourceAvailability = await readFile(new URL("../skills/arcforge/references/source-availability-policy.md", import.meta.url), "utf8");
  const projectApplicability = await readFile(new URL("../skills/arcforge/references/project-applicability-policy.md", import.meta.url), "utf8");
  const skillProjectSchema = JSON.parse(await readFile(new URL("../skills/arcforge/references/skill-project-manifest.schema.json", import.meta.url), "utf8"));
  ...
  assert.match(skill, /来源类型策略/);
  assert.match(skill, /source-availability-policy\.md/);
  assert.match(skill, /project-applicability-policy\.md/);
  assert.match(skill, /不按技术栈、文件标记或固定项目类别自动判定/);
  assert.match(skill, /project availability plan/);
  assert.match(skill, /planDigest/);
  assert.match(skill, /本次 `--availability` 覆盖不能替代维护源配置/);
  assert.match(agentYaml, /\$arcforge/);
  assert.match(agentYaml, /来源、维护源、应用目标、共享目标/);
  assert.match(agentYaml, /关系记录/);
  assert.match(agentYaml, /arcforge\.skill-project\.json/);
  assert.match(agentYaml, /project availability plan/);
  assert.match(agentYaml, /planDigest/);
  assert.match(agentYaml, /projectApplicability/);
  assert.match(agentYaml, /实际证据动态判断/);
  assert.match(agentYaml, /真实写入、覆盖、Git 和远程动作都须确认/);
  assert.ok(agentYaml.length < 1000, `ArcForge agent metadata is too long: ${agentYaml.length}`);
  ...
  assert.match(sourceAvailability, /user-ambient/);
  assert.match(sourceAvailability, /project-ambient/);
  assert.match(sourceAvailability, /user-on-demand/);
  assert.match(sourceAvailability, /不使用项目级按需目录/);
  assert.match(sourceAvailability, /project availability run/);
  assert.match(sourceAvailability, /只影响本次解析/);
  assert.match(sourceAvailability, /projectApplicability/);
  assert.match(sourceAvailability, /不保存固定技术栈或项目类型枚举/);
  assert.match(projectApplicability, /只定义通用数据结构、校验规则和 Agent 判断方法/);
  assert.match(projectApplicability, /不假设目标一定是软件代码仓库/);
  assert.match(projectApplicability, /suitable/);
  assert.match(projectApplicability, /unsuitable/);
  assert.match(projectApplicability, /needs-input/);
  assert.match(projectApplicability, /overridden/);
  assert.match(projectApplicability, /不写回维护源/);
  assert.equal(skillProjectSchema.$schema, "https://json-schema.org/draft/2020-12/schema");
  assert.deepEqual(skillProjectSchema.$defs.availabilityMode.enum, ["user-ambient", "project-ambient", "user-on-demand"]);
  assert.deepEqual(skillProjectSchema.$defs.applicabilityCondition.properties.kind.enum, ["required", "preferred", "excluded"]);
  assert.equal(skillProjectSchema.$defs.skillPolicy.allOf[1].then.properties.mode.const, "project-ambient");
  ...
  assert.match(cli, /apply plan.*--agent-targets/);
  assert.match(cli, /透传维护源的 `projectApplicability`/);
  assert.match(cli, /CLI 只校验结构、condition 绑定与项目根覆盖，不运行固定分类器/);
