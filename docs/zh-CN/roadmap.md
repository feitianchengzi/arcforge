...
ArcForge 会按真实需求推进。`0.x` 系列应该保持很小，只做让当前 local-first 工作流可靠闭环所必需的事情。更大的工作流只有在出现足够真实使用、star、issue 或 PR 时，才值得继续投入维护成本。

这里没有固定日期。star、具体 issue 和有用的 PR，是决定下一步做什么的主要信号。

## 方向

ArcForge 应该继续作为 AI agent skills 的生命周期治理层，覆盖创建验证之后到发布准备之前的本地流程：

```text
编写/迭代 skill -> 子代理验证 -> 审计 -> 组成 profile -> 应用到目标 -> 检查 drift -> 发布准备
```

它应该保持 local-first 和 GitHub-first。Git 继续作为 review、历史、release 和团队共享的 source of truth。

ArcForge 不应该变成托管 marketplace、公开 registry、公开搜索、评分系统、付费分发平台或 agent runtime。发现、分发和运行时加载已经有 registry 和 installer 负责。ArcForge 负责更早的环节。
...
- 保持 `arcforge-skill-first` 能把工作模式沉淀成 skill，并在验证通过后引导进入 ArcForge 治理流程
- 保持审计输出足够清楚，可用于共享前检查
...
- 基于真实误报或漏报的小范围审计规则调整
- 基于真实使用反馈小范围改进 Skill First 的前测、复测和交接说明

## 1.x - 稳定的个人和小团队使用

目标：让个人开发者或小团队可以依赖 ArcForge，而不需要担心本地项目状态结构频繁破坏。
...
- 面向公开 registry 的发布就绪助手，但 registry 逻辑不进入 ArcForge 核心
- 以 adapter 方式集成 installer，而不是替代 installer

这些都应该是可选层，不应该把 ArcForge 做成 marketplace、registry 或 runtime。
