...
ArcForge 区分来源内容和本地用户状态。日常项目设置保存在用户级项目状态 `~/.arcforge/projects`。当项目根目录存在 `arcforge.config.json` 时，系统按需迁移到用户级项目状态，然后从来源 checkout 删除该文件。
...
应用启动时，主进程会安装或修复用户级 `arcforge` shim。环境检测会报告 shim 路径、PATH 可见性、Git 可用性和可选集成工具状态。
